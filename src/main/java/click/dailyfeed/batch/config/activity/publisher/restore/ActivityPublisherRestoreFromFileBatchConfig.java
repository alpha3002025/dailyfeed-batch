...
public class ActivityPublisherRestoreFromFileBatchConfig {
    ...
    private static class ProcessedItem {
        ...
        private final RedisDLQDocument document;

        public ProcessedItem(File sourceFile, File editingFile, RedisDLQDocument document) {
        ...
        public RedisDLQDocument getDocument() {
            return document;
        }
    }

    @Bean
    public Job activityKafkaFailureRecoveryFromFileJob(JobRepository jobRepository, Step activityKafkaFailureRecoveryFromFileStep) {
        return new JobBuilder("activityKafkaFailureRecoveryFromFileJob", jobRepository)
                .start(activityKafkaFailureRecoveryFromFileStep)
                .build();
    }

    @Bean
    public Step activityKafkaFailureRecoveryFromFileStep(JobRepository jobRepository,
                                                 PlatformTransactionManager transactionManager,
                                                 MultiResourceItemReader<File> activityKafkaFailureFileReader,
                                                 ItemProcessor<File, ProcessedItem> activityKafkaFailureFileProcessor,
                                                 ItemWriter<ProcessedItem> activityKafkaFailureMongoWriter) {
        return new StepBuilder("activityKafkaFailureRecoveryFromFileStep", jobRepository)
                .<File, ProcessedItem>chunk(10, transactionManager)
                .reader(activityKafkaFailureFileReader)
                .processor(activityKafkaFailureFileProcessor)
                .writer(activityKafkaFailureMongoWriter)
                .build();
    }

    @Bean
    @StepScope
    public MultiResourceItemReader<File> activityKafkaFailureFileReader(
            @Value("#{jobParameters['failurePath'] ?: '${batch.publisher-file-path:kafka-failure-storage}'}") String failurePath) {
    ...
    @Bean
    @StepScope
    public ItemProcessor<File, ProcessedItem> activityKafkaFailureFileProcessor() {
        return originalFile -> {
            ...
            try {
                ...
                // RedisDLQDocument 생성
                RedisDLQDocument redisDLQDocument = RedisDLQDocument.newRedisDLQ(
                        memberActivityMessage.getKey(),
                        jsonPayload
                );

                // File과 Document를 함께 반환 (삭제는 Writer에서 수행)
                return new ProcessedItem(originalFile, editingFile, redisDLQDocument);
            } catch (Exception e) {
    ...
    @Bean
    @StepScope
    public ItemWriter<ProcessedItem> activityKafkaFailureMongoWriter() {
        return chunk -> {
            ...
            try {
                ...
                log.info("Saved {} RedisDLQDocuments to MongoDB", documentsToSave.size());
